...
from bot.features.progression.rendering.process_worker import (
...
class RenderManager:
    ...
    def _get_cache_key(self, ctx: RenderContext) -> str:
        ...
        avatar_hash = hashlib.sha1(
            ctx.avatar_bytes[:256] if ctx.avatar_bytes else b""
        ).hexdigest()[:16]
        ...
        async with self._render_semaphore:
            ...
            try:
                ...
                if img_bytes:
                    ...
                    print(
                        f"[RenderManager] Rendered and cached profile for {ctx.display_name}"
                    )
    ...
    async def render_leaderboard(
        self, rows_data: list, exp_icon_path: str, cache_key: str
    ) -> Optional[bytes]:
