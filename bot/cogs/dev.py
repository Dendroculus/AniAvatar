...
from bot.config.configs import OWNER_ID
...
class DevCommands(commands.Cog):
    ...
    @commands.hybrid_command(
        name="manipulate_profile", description="Dev: Modify coins and EXP for a user."
    )
    async def manipulate_profile(
        ...
        embed.add_field(
            name="Coins",
            value=f"{current_coins:,} ({'+' if coins > 0 else ''}{coins})",
            inline=True,
        )
        embed.add_field(
            name="Level",
            value=f"{current_level} (Leveled Up: {leveled_up})",
            inline=True,
        )
