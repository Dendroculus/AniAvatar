...
class GambleView(discord.ui.View):
    ...
    def __init__(
        ...
        *,
        ...
        self.exit_button = discord.ui.Button(
            label="Exit Gamble", style=discord.ButtonStyle.danger
        )
        ...
        if self.message:
            try:
                await self.message.edit(
                    content="❌ Gamble timed out.", embed=None, view=None
                )
    ...
    def _parse_value_from_interaction(
    ...
    async def _send_invalid_selection(self, interaction: discord.Interaction) -> None:
        ...
        await self.fun._send(
            self.ctx, interaction, "❌ Invalid selection.", ephemeral=True
        )
    ...
    async def _handle_bet_value(
        ...
        if bet <= 0:
            await self.fun._send(
                self.ctx, interaction, "❌ Invalid bet amount.", ephemeral=True
            )
            await self._clear_selection()
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != self.user_id:
            await self.fun._send(
                self.ctx, interaction, FC.FALSE_GAMBLE_SESSION, ephemeral=True
            )
            return
        ...
        try:
            if not interaction.response.is_done():
                await interaction.response.edit_message(
                    content="❌ Gamble exited.", embed=None, view=None
                )
            else:
