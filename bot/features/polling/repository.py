...
async def _set_stmt_timeout(
    conn: asyncpg.Connection, ms: int = POLCONST.SAFETY_TIMEOUT_MS
):
    """
...
async def upsert_vote(
    pool: asyncpg.Pool, message_id: int, user_id: int, option_idx: int
):
    """
...
async def load_active_polls(pool: asyncpg.Pool):
    ...
    options_map: Dict[int, List[str]] = {
        r["message_id"]: json.loads(r["options"]) for r in poll_rows
    }
...
async def record_poll_result(
    pool: asyncpg.Pool, message_id, winners, counts, total_votes
):
    """
